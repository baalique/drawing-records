...
@pytest.mark.e2e
def test_create_success(test_client, create_drawing_dto):
    with test_client:
...
@pytest.mark.e2e
def test_get_one_success(test_client, create_drawing_dto):
    with test_client:
...
@pytest.mark.e2e
def test_get_all(test_client, create_many_drawings_dto):
...
@pytest.mark.e2e
def test_update_success(test_client, create_drawing_dto, update_drawing_dto):
    with test_client:
        create_response = test_client.post(
            "drawing/new", json=jsonable_encoder(create_drawing_dto)
        )
...
@pytest.mark.e2e
def test_update_fails_not_found(test_client, update_drawing_dto):
    with test_client:
        response = test_client.patch(
...
def test_update_wrong_data_not_affect(
    test_client, create_drawing_dto, update_drawing_dto
):
    with test_client:
        create_response = test_client.post(
            "drawing/new", json=jsonable_encoder(create_drawing_dto)
        ...
        update_response = test_client.patch(f"drawing/{id_}", json={"wrong data": 0})
...
@pytest.mark.e2e
def test_delete_success(test_client, create_drawing_dto):
    with test_client:
        create_response = test_client.post(
            "drawing/new", json=jsonable_encoder(create_drawing_dto)
        )
